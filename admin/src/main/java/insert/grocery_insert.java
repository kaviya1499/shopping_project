...
@MultipartConfig
@WebServlet("/grocery_insert")
public class grocery_insert extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    public grocery_insert() {
        super();
    }
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try 
		{
			...
			String query="insert into grocery(name,price,about,image) values(?,?,?,?)";
			...
			if(row>0)
			{
				  response.sendRedirect("grocery_table.jsp");
			}
			
			else
			{
				  response.sendRedirect("grocery_form.jsp");
			}
