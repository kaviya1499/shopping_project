...
@MultipartConfig
@WebServlet("/health_insert")
public class health_insert extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    public health_insert() {
        super();
    }
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try 
		{
			...
			String query="insert into healthcare(name,price,discription,image) values(?,?,?,?)";
			...
			if(row>0)
			{
				  response.sendRedirect("health_table.jsp");
			}
			
			else
			{
				  response.sendRedirect("health_form.jsp");
			}
