...
@MultipartConfig
@WebServlet("/gift_insert")
public class gift_insert extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try 
		{
			...
			String query="insert into gift(name,price,discription,image) values(?,?,?,?)";
			...
			if(row>0)
			{
				  response.sendRedirect("gift_table.jsp");
			}
			
			else
			{
				  response.sendRedirect("gift_form.jsp");
			}
