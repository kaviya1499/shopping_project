...
@MultipartConfig
@WebServlet("/electronic_insert")
public class electronic_insert extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    public electronic_insert() {
        super();
    }
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try 
		{
			...
			String query="insert into electronic(name,price,discription,image) values(?,?,?,?)";
			...
			if(row>0)
			{
				  response.sendRedirect("electronics_table.jsp");
			}
			
			else
			{
				  response.sendRedirect("electronics_form.jsp");
			}
