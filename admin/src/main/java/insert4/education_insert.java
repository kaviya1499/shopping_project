...
@MultipartConfig
@WebServlet("/education_insert")
public class education_insert extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    public education_insert() {
        super();
    }
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try 
		{
			...
			String query="insert into education(name,price,discription,image) values(?,?,?,?)";
			...
			if(row>0)
			{
				  response.sendRedirect("education_table.jsp");
			}
			
			else
			{
				  response.sendRedirect("education_form.jsp");
			}
