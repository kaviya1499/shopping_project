...
@MultipartConfig
@WebServlet("/sports_insert")
public class sports_insert extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    public sports_insert() {
        super();
    }
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try 
		{
			...
			String query="insert into sports(name,price,discription,image) values(?,?,?,?)";
			...
			if(row>0)
			{
				  response.sendRedirect("sports_table.jsp");
			}
			
			else
			{
				  response.sendRedirect("sports_form.jsp");
			}
